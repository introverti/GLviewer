...
void render(float dt) {
  ...
  CHECK_GL(glColor4f(00.0f, 0.0f, 0.1f, 1.0f));
...
void derivative(StarArray const &stars, StarArray &d_stars) {
  for (size_t i = 0; i < stars.size(); i++) {
    ...
    for (size_t j = 0; j < stars.size(); j++) {
      ...
      auto rnorm = r12 / rlen;
      // 引力大小为：F = G*m1*m2/(|r|*|r|)，方向为 -r，其中 r
      // 为两个质点之间距离，r = p1 - p2
...
void integrate(StarArray &out_stars, StarArray const &stars,
               StarArray const &d_stars, float dt) {
  for (size_t i = 0; i < stars.size(); i++) {
...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(mode->width, mode->height, "Trisolar", NULL, NULL);
